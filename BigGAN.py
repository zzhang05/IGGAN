...
class G_D(nn.Module):
    ...
    def forward(self, z, gy, x=None, real_fake_x = None, dy=None, train_G=False, return_G_z=False, policy=False, CR=False, CR_augment=None):
        if z is not None:
        ...
        if G_z is None:
            ...
            if CR:
                return torch.split(D_out, [G_z.shape[0], x.shape[0], x.shape[0]]), D_real_fake
            else:
                return torch.split(D_out, [G_z.shape[0], x.shape[0]]), D_real_fake
        else:
            if return_G_z:
                return D_out, G_z
            else:
